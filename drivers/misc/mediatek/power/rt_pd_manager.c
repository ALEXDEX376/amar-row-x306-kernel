					...
					unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_POGO_EN:
		pr_info("%s pogo en = %d\n", __func__, noti->en_state.en);
		if(noti->en_state.en == 1)
		{
			charger_dev_pogo_enable(primary_charger,true);
		}
		else
		{
			charger_dev_pogo_enable(primary_charger,false);
		}
		/* active low */
		//gpio_set_value(pogo_en_gpio, !noti->en_state.en);
		break;
		case TCP_NOTIFY_IRQ_ENABLED:
		pr_info("%s irq enabled\n", __func__);
		if (tcpc_kpoc) {
			tcpm_typec_change_role(tcpc_dev, TYPEC_ROLE_SNK);
			tcpm_set_dpm_caps(tcpc_dev,
					  tcpm_inquire_dpm_caps(tcpc_dev) |
...
static int rt_pd_manager_probe(struct platform_device *pdev)
{
	...
	if(tcpc_kpoc)
	{
		...
		tcpm_set_dpm_caps(tcpc_dev, tcpm_inquire_dpm_caps(tcpc_dev) |
				  DPM_CAP_PR_SWAP_REJECT_AS_SRC);
	}
