...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (adapter_dev_get_property(info->pd_adapter, TYPEC_RP_LEVEL)
			== 3000) {
			printk("===leon==charger=======7777============\r\n");

			pdata->input_current_limit = 3000000;
			pdata->charging_current_limit = 3000000;
		} else if (adapter_dev_get_property(info->pd_adapter,
			TYPEC_RP_LEVEL) == 1500) {
			printk("===leon==charger=======8888============\r\n");

			pdata->input_current_limit = 1500000;
			pdata->charging_current_limit = 2000000;
		} else {
		printk("===leon==charger=======9999============\r\n");
		...
		printk("===leon==charger=======mtk_pdc_check_charger============\r\n");
		ret = mtk_pdc_get_setting(info, &vbus, &cur, &idx);
		if (ret != -1 && idx != -1) {
			printk("===leon==charger=======aaaa============\r\n");
...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita)
	{
		...
		printk("====================================[SW_JEITA]======set vol[%d]=========leon======\n",constant_voltage);
		return;
	}
	/* dynamic cv*/
