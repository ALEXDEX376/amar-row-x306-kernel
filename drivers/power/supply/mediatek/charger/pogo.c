...
static int dock_pogo_probe(struct platform_device *pdev)
{
	...
	if (request_irq(irq_dock, dock_interrupt, IRQ_TYPE_EDGE_BOTH, "pogo_dock_irq", NULL)) {}
	if (request_irq(irq_usb, usb_interrupt, IRQ_TYPE_EDGE_BOTH, "pogo_usb_irq", NULL)) {}
	primary_charger = get_charger_by_name("primary_chg");
	if (!primary_charger) {
		printk("==========pogo driver: get primary charger device failed\n");
	}

  	if (power_supply_register(&pdev->dev, &dock_pogo_desc,NULL)) {}
    return 0;
}
