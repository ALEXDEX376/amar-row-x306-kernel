...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	printk("====================================[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d\n",
		sw_jeita->pre_sm, sw_jeita->sm, battery_temp,
		sw_jeita->cv);
}
...
static void charger_update_data(struct charger_manager *info)
{
	...
	if (ext_psy)
	{
		...
		if(reta.intval == -99)
		{
			...
			printk("=========battery temp from system[%d]================\r",info->battery_temp);
		}
		else
		{
			printk("=========battery temp from mm8013[%d]================\r",reta.intval);
			if(reta.intval != 2550)
			{
				info->battery_temp = reta.intval/10;
			}
		}
	}
	...
	if (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
		...
		if (atomic_read(&info->enable_kpoc_shdn)) {
			...
			if (vbus >= 0 && vbus < 2500 && !mt_charger_plugin()) {
				...
				{
					if(!check_dock_charger())
					{
						printk("===========leon=======kpoc_power_off_check === power off ====\n");
						msleep(3000);
						kernel_power_off();
					}
					else
					{
						printk("===========leon=======kpoc_power_off_check === dock in missed====\n");
					}
				}
			}
		}
				...
				struct device *dev)
{
	...
	info->data.pd_charger_current = 2050000;
...
{	
	...
	struct switch_charging_alg_data *swchgalg = info->algorithm_data;
		...
		chr_err("batt_charging_enable:%d\n",en);
	if (en && (swchgalg->state != CHR_CC && (mt_get_charger_type() !=CHARGER_UNKNOWN))) {
		charger_dev_hz_mode(info->chg1_dev, 0);
