...
static int himax_switch_mode_inspection(int mode)
{
	...
	/*Stop Handshaking*/
	himax_in_parse_assign_cmd(sram_adr_rawdata_addr, tmp_addr,
			sizeof(tmp_addr));
...
static void himax_neg_noise_sup(uint8_t *data)
{
	...
	himax_in_parse_assign_cmd(addr_neg_noise_sup, tmp_addr,
		sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);

	if ((tmp_data[3] & 0x04) == 0x04) {
		himax_in_parse_assign_cmd(data_neg_noise, tmp_data,
			sizeof(tmp_data));
...
static void himax_get_noise_base(uint8_t checktype)/*Normal Threshold*/
{
	...
	switch (checktype) {
	case HIMAX_WEIGHT_NOISE:
		himax_in_parse_assign_cmd(addr_normal_noise_thx,
				tmp_addr, sizeof(tmp_addr));
		break;
	case HIMAX_LPWUG_WEIGHT_NOISE:
		himax_in_parse_assign_cmd(addr_lpwug_noise_thx,
				tmp_addr, sizeof(tmp_addr));
		break;
	default:
	...
	himax_in_parse_assign_cmd(addr_recal_thx,
			tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
...
static uint16_t himax_get_palm_num(void)/*Palm Number*/
{
	...
	himax_in_parse_assign_cmd(addr_palm_num,
			tmp_addr, sizeof(tmp_addr));
	g_core_fp.fp_register_read(tmp_addr, 4, tmp_data, false);
...
static int himax_get_noise_weight_test(uint8_t checktype)
{
	...
	himax_in_parse_assign_cmd(addr_weight_sup,
			tmp_addr, sizeof(tmp_addr));
	...
	switch (checktype) {
	case HIMAX_WEIGHT_NOISE:
		himax_in_parse_assign_cmd(addr_normal_weight_a,
				tmp_addr, sizeof(tmp_addr));
		break;
	case HIMAX_LPWUG_WEIGHT_NOISE:
		himax_in_parse_assign_cmd(addr_lpwug_weight_a,
				tmp_addr, sizeof(tmp_addr));
		break;
	default:
		I("%s Not support type\n", __func__);
	...
	if (on) {
		...
		if (strcmp(HX_83102D_SERIES_PWON, private_ts->chip_name) == 0) {
			...
			do {
				...
				I("%s: now read[2]=0x%02X\n", __func__,
					tmp_read[2]);
				I("%s: now read[1]=0x%02X\n", __func__,
					tmp_read[1]);
				I("%s: now read[0]=0x%02X\n", __func__,
					tmp_read[0]);
	...
	while (g_hx_head_str[i]) {
		...
		if (start_ptr == NULL || end_ptr == NULL)
			...
			strlen(g_hx_head_str[i])) == 0) {
				...
				I("%s,Start to parse %s\n", __func__,
					g_hx_head_str[i]);
				ret |= himax_parse_criteria_file(start_ptr
					+ start_str_len,
					str_size);
			}
		}
...
static int himax_chip_self_test(void)
{
	...
	uint32_t test_size = ic_data->HX_TX_NUM
			* ic_data->HX_RX_NUM
			+ ic_data->HX_TX_NUM
			+ ic_data->HX_RX_NUM;
	int i = 0;
